...
namespace {
class USRGenVisitor : public RecursiveASTVisitor<USRGenVisitor> {
private:
  ASTContext *Context;
...
public:
  USRGenVisitor(ASTContext *Context, StringRef OutputFile, bool ShowDiff)
  ...
  void ProcessDecl(const clang::Decl *Decl, StringRef DeclName) {
    ...
    SmallString<256> ArmorUSR;
    bool ArmorSuccess = armor::generateUSRForDecl(Decl, ArmorUSR);

    if (ShowDiff && ArmorUSR.equals(ClangUSR)) return;
  ...
  ~USRGenVisitor() { os_output.close(); }
};

class USRGenConsumer : public ASTConsumer {
private:
  USRGenVisitor Visitor;
public:
  explicit USRGenConsumer(ASTContext *Context, const std::string &OutputFile, bool ShowDiff)
...
class USRGenAction : public ASTFrontendAction {
private:
  std::string OutputFile;
  bool ShowDiff;
public:
  USRGenAction(const std::string &OutputFile, bool ShowDiff) : OutputFile(OutputFile), ShowDiff(ShowDiff) {}

  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) override {
    return std::make_unique<USRGenConsumer>(&CI.getASTContext(), OutputFile, ShowDiff);
  }
};

class USRGenActionFactory : public FrontendActionFactory {
private:
  std::string OutputFile;
  bool ShowDiff;
public:
  USRGenActionFactory(const std::string &OutputFile, bool ShowDiff) : OutputFile(OutputFile), ShowDiff(ShowDiff) {}

  std::unique_ptr<FrontendAction> create() override {
    return std::make_unique<USRGenAction>(OutputFile, ShowDiff);
  }
};
...
int main(int argc, const char **argv) {
  ...
  auto Factory = std::make_unique<USRGenActionFactory>(outputFile, showDiff);
