...
constexpr float GLOBAL_PI = 3.14159265358979323846;
constexpr char GLOBAL_VERSION[10] = "v2.0.1";

// Global type definitions outside namespace
typedef signed int int32_t;

// Global utility function declarations
bool isValidInput(const std::string& input, int ok);
void logMessage(int logLevel = 0);
...
struct diag_callback_tbl_t {
    int inited;
    int (*cb_func_ptr)(unsigned char *, int len);
    void *context_data;
};
...
constexpr float PACKET_BUFFER_SIZE = MAX_PACKET_SIZE;
...
struct alpha {
    ...
    int processor;  // Using ProcessorType from custom_defs.h
...
double calculateArea(double radius, int a);
double calculateArea(double breath);
...
ProcessorType getOptimalProcessor(const DataPacket& packet, int a);
AlphaData processAlphaData(const std::string& input);
Point findOptimalPoint(const std::vector<Point>& point);
...
template<>
constexpr int DEFAULT_VALUES<float> = 0;
...
template<typename T>
constexpr int is_numeric = std::is_arithmetic<T>::value;
...
public:
    ...
    bool handlePacket(const DataPacket& packet, int a) {
...
template<typename T>
custom::ConfigOptions GlobalContainer<T>::globalConfigs = {false, false, 0, ""};
