...
uint64_t FibonacciHash::hashFromSourceRange(clang::SourceManager* SM, clang::SourceRange Range) {
    ...
    // If either location is inside a macro expansion (e.g. NULL is defined as
    // __null in C++ mode inside <stddef.h>), getFileOffset would return an offset
    // relative to the macro's own buffer — not the main file. getExpansionRange
    // resolves both ends to where the macro is written in the main file.
    if (StartLoc.isMacroID() || EndLoc.isMacroID()) {
        clang::CharSourceRange ExpRange = SM->getExpansionRange(
            clang::SourceRange(StartLoc, EndLoc));
        StartLoc = ExpRange.getBegin();
        EndLoc   = ExpRange.getEnd();
    }
